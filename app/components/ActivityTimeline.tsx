...
const eventIcons: Record<TimelineEventType, typeof LuGitCommitVertical> = {
  commits: LuGitCommitVertical,
  prs: LuGitPullRequest,
  issues: LuCircleAlert,
  repos: LuFolderKanban,
  stars: LuStar,
};
  ...
  return (
    <div className="rounded-2xl border border-[#e2d6c8] bg-[#fbf7f0] p-2.5 sm:p-6 shadow-sm overflow-hidden">
      <div className="mb-2.5 sm:mb-4">
        <h3 className="text-base sm:text-lg font-semibold text-[#2f2a24] mb-2">
          Activity Timeline
        </h3>
        ...
        <div className="flex flex-wrap gap-1.5 sm:gap-2 mb-3 sm:mb-4">
          {(["all", "commits", "prs", "issues", "repos", "stars"] as const).map((type) => {
            const Icon = type === "all" ? LuCalendarDays : eventIcons[type];
            return (
              <button
                key={type}
                type="button"
                onClick={() => setSelectedFilter(type)}
                className={`flex items-center gap-1 sm:gap-1.5 rounded-full border px-2 sm:px-3 py-0.5 sm:py-1 text-[0.65rem] sm:text-xs font-medium transition ${
                  selectedFilter === type
      ...
      {!data || filteredEvents.length === 0 ? (
        <div className="rounded-xl border border-[#e2d6c8] bg-[#f3ede4] p-4 sm:p-6 text-center text-xs sm:text-sm text-[#6f665b]">
          {!data
            ? "No activity timeline data available."
        ...
        <>
          <div className="space-y-2 sm:space-y-3 max-h-96 overflow-y-auto pl-2 sm:pl-3 pr-1 -mr-1">
            {filteredEvents.slice(0, 75).map((event, index) => {
              ...
              return (
                <div
                  key={event.id}
                  className={`flex gap-2 sm:gap-4 relative pl-6 sm:pl-8 ${
                    index < filteredEvents.length - 1
                      ? "border-l-2 border-[#e2d6c8] pb-2 sm:pb-3"
                      : ""
                  }`}
                >
                  {/* Icon */}
                  <div
                    className={`absolute -left-[10px] sm:-left-[14px] top-0 rounded-full border-2 p-1 sm:p-1.5 ${colorClass} z-10`}
                  >
                  ...
                  <div className="flex-1 min-w-0 pb-1 overflow-hidden">
                    <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-1 sm:gap-3 mb-1 sm:mb-1.5">
                      <h4 className="text-xs sm:text-sm font-semibold text-[#2f2a24] flex-1 leading-tight break-words">
                        {event.title}
                      </h4>
                      <span className="text-[0.65rem] sm:text-xs text-[#6f665b] whitespace-nowrap flex-shrink-0">
                    ...
                    {event.repo && (
                      <div className="flex items-center gap-1.5 sm:gap-2 mb-1 sm:mb-1.5 flex-wrap">
                        <span className="inline-flex items-center gap-1 text-[0.65rem] sm:text-xs text-[#6f665b] font-medium bg-[#f3ede4] px-1.5 sm:px-2 py-0.5 rounded">
                          {event.repo}
                        </span>
                        {event.actor && (
                          <span className="text-[0.65rem] sm:text-xs text-[#5f564d]">
                    ...
                    {event.description && (
                      <p className="text-[0.65rem] sm:text-xs text-[#5f564d] mb-1 sm:mb-2 leading-relaxed line-clamp-2 break-words">
                        {event.description}
                      </p>
                    )}
                    ...
                    {event.url && (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="inline-flex items-center gap-1 text-[0.65rem] sm:text-xs text-[#4f6d6a] hover:text-[#3f5d5a] underline transition-colors break-all"
                      >
                        View on GitHub
                        <span>→</span>
                      </a>
          ...
          {filteredEvents.length > 75 && (
            <p className="mt-3 sm:mt-4 text-[0.65rem] sm:text-xs text-[#6f665b] text-center">
